...
async def application_lifespan(app: FastAPI):
    ...
    try:
        if os.path.exists(local_json_path):
            ...
            logger.info(f"✅ [CandleFlow Core] Loaded {len(GLOBAL_TICKER_CACHE)} equities from disk.")
        else:
            raise FileNotFoundError(f"Master dataset missing at: {local_json_path}")

    except Exception as e:
        logger.error(f"🚨 Local filesystem init failure: {e}. Using fallback array.")
        fallback_raw = {
        ...
        logger.info(f"✅ Fallback array loaded: {len(GLOBAL_TICKER_CACHE)} nodes.")
...
# ----------------------------------------------------------------------
# NOTE: PaperBroker is intentionally NOT wired into the signal route.
# The signal endpoint is a pure read — it analyses and returns a signal.
# Paper trading is a separate concern handled via /api/portfolio routes.
# ----------------------------------------------------------------------
...
if HAS_TF and os.path.exists(LSTM_PATH):
    try:
        ...
        print("🤖 Dual-Class Sigmoid Engine mounted successfully!")
    except Exception as e:
...
def fetch_local_indian_news(ticker_symbol):
    ...
    try:
        ...
        return [
            {
                "headline": item.find('title').text.rsplit(' - ', 1)[0],
...
def search_tickers_dynamically(q: str = ""):
    ...
    return sorted(prefix_matches, key=lambda x: x["name"])[:8]
...
# ----------------------------------------------------------------------
# 📈 ROUTE: PRICE HISTORY  (used by PriceChart.jsx)
# ----------------------------------------------------------------------
...
# ----------------------------------------------------------------------
# 📡 CORE SIGNAL ROUTE  — pure analysis, zero side effects
# ----------------------------------------------------------------------
...
def get_stock_full_info(query: str):
    try:
        ...
        logger.info(f"[{best_ticker}] raw={raw_pred:.4f} p_buy={p_buy:.4f} p_sell={p_sell:.4f} spread={net_spread:.4f} bb={current_bb_width:.2f}%")

        # ── RULE 1: VOLATILITY COMPRESSION OVERRIDE ──
        if current_bb_width < 3.0:
            ai_signal_text  = "HOLD"
            confidence_band = f"Volatility Squeeze Active ({current_bb_width:.1f}% Compression) // Range Bound Trap"

        # ── RULE 2: DIRECTIONAL PROBABILITY SPREAD ──
        else:
            if direction == "BUY":
                if net_spread >= 0.08:
        ...
        # ── RULE 3: MACRO PROTECTION OVERRIDE ──
        ...
        # ── SIGNAL QUALITY LABEL (replaces broker execution status) ──
        # Describes signal quality without any side effects
        if net_spread >= 0.08:
            signal_quality = "High Conviction — Strong directional spread detected"
        elif net_spread >= 0.04:
            signal_quality = "Moderate Conviction — Directional bias present"
        elif net_spread >= 0.01:
            signal_quality = "Low Conviction — Weak edge, exercise caution"
        else:
            signal_quality = "No Edge — Model is near-neutral on this ticker"
        ...
        return {
            ...
            "signal_quality":   signal_quality,       # ← replaces position_status
            "net_spread":       round(net_spread, 4),  # ← exposed for frontend tiers
