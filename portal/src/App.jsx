...
import {
  TrendingUp,TrendingDown,ShieldAlert,Activity,Search,Newspaper,BarChart3,Radio,
  Percent,AlertTriangle,PlusCircle,Trash2
} from 'lucide-react';
...
export default function App() {
  ...
  useEffect(() => {
    fetch('http://localhost:8000/api/user-portfolio')
      .then(res => res.json())
    ...
    const delayDebounceThread = setTimeout(async () => {
      try {
        const response = await fetch(
          `http://localhost:8000/api/tickers/search?q=${encodeURIComponent(cleanInput)}`
        );
  ...
  const fetchStockAnalysis = async (queryStr) => {
    ...
    try {
      const response = await fetch(
        `http://localhost:8000/api/stock-info/${queryStr}`
      );
      ...
      setError(
        "Failed to communicate with CandleFlow API gateway. Verify backend is running."
      );
      setStockData(null);
    } finally {
      setLoading(false);
...
const toggleWatchlist = async (ticker) => {
  ...
  try {
    ...
    await fetch(
      'http://localhost:8000/api/user-portfolio/toggle',
      {
        method: 'POST',
        headers: {
...
const handleViewAnalysis = (ticker) => {
 ...
 useEffect(() => {
  ...
  const handleSearchSubmit = (e) => {
    ...
    const targetQuery = typedInput.trim()
      ? typedInput.trim().toUpperCase()
      : searchQuery;

    if (targetQuery) {
      const standardQuery = targetQuery.endsWith('.NS')
        ? targetQuery
        : `${targetQuery}.NS`;
  ...
  const getSignalBadgeStyles = (signal) => {
    ...
    if (upperSig.includes("STRONG BUY"))
      return "bg-emerald-950 text-emerald-400 border-emerald-500 animate-pulse";

    if (upperSig.includes("BUY"))
      return "bg-green-950 text-green-400 border-green-600";

    if (upperSig.includes("STRONG SELL"))
      return "bg-rose-950 text-rose-400 border-rose-500 animate-pulse";

    if (upperSig.includes("SELL"))
  ...
  const computeRiskProfile = (data) => {
    ...
    let sizing =
      "0% Capital Weight - Stand Aside (Cash Reserve)";
    ...
    return {
      ...
      takeProfit:
        takeProfit > 0
          ? `₹${takeProfit.toLocaleString('en-IN', {
              maximumFractionDigits: 2
            })}`
          : "Stand Aside",
  ...
  const riskProfile = stockData
    ? computeRiskProfile(stockData)
    : null;
        ...
        <div className="flex items-center space-x-3">
          ...
          <div>
            ...
            <p className="text-xs text-slate-500 font-mono">
              v2.5 // Live Paper-Trading Matrix Terminal
            </p>
          </div>
        </div>
              ...
              {suggestions.map((item) => (
                ...
                >
                  <div className="flex flex-col truncate pr-2">
                    <span className="text-sm font-bold text-slate-200 group-hover:text-emerald-400 truncate">
                      {item.name}
                    </span>
                  </div>
          ...
          <div className="bg-rose-950/40 border border-rose-800/60 text-rose-300 p-4 rounded-xl flex items-start space-x-3">
            ...
            <div>
              <span className="font-semibold text-sm">
                Pipeline Exception Trace:
              </span>

              <p className="text-xs text-rose-400/90 mt-0.5 font-mono">
                {error}
              </p>
            </div>
          </div>
              ...
              <div className="lg:col-span-2 space-y-6">
                ...
                <div className="bg-slate-900/60 border border-slate-900 rounded-2xl p-6 flex flex-col md:flex-row md:items-center justify-between gap-4">
                  ...
                  <div className="text-right flex md:flex-col justify-between items-center md:items-end">
                    ...
                    <div className={`flex items-center space-x-1 font-mono text-sm font-semibold mt-1 ${
                      stockData.daily_change >= 0
                        ? 'text-emerald-400'
                        : 'text-rose-400'
                    }`}>
                      {stockData.daily_change >= 0
                      ...
                      <span>
                        {stockData.daily_change >= 0 ? '+' : ''}
                        {stockData.daily_change?.toFixed(2)}
                ...
                <div className="bg-slate-900/30 border border-slate-900 rounded-2xl p-6">
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4 font-mono">
                    ...
                    {Object.entries(stockData.metrics || {}).map(([key, val]) => (
                      ...
                      <div
                        key={key}
                        className="bg-slate-950/60 border border-slate-900 p-3.5 rounded-xl"
                      >
                        <span className="text-[10px] text-slate-500 block uppercase tracking-tight truncate">
                          {key.replace(/_/g, ' ')}
                        </span>
